...
const validateEmail = (email) => {
    ...
    if (!emailRegex.test(email))
        throw ApiError.badRequest("Invalid email format. Please enter a valid email address.");
};
...
export const signUp = async (req, res) => {
    try {
        ...
        if (!fullname || !email || !password || !phone)
            throw ApiError.badRequest("fullname, email, phone and password are required.");
...
export const signIn = async (req, res) => {
    try {
        ...
        if (!password || (!email && !phone))
            throw ApiError.badRequest("Either email or phone, and password are required.");
        ...
        if (!existingUser)
            throw ApiError.notFound("User not found. Please check your email or phone.");
...
export const updateProfile = async (req, res) => {
    try {
        ...
        if (!fullname && !phone && (!currentPassword || !newPassword))
            throw ApiError.badRequest("At least one field (fullname, phone, or currentPassword + newPassword) is required.");
        
        if ((currentPassword && !newPassword) || (!currentPassword && newPassword))
             throw ApiError.badRequest("Both current password and new password are required for password change.");

        const { rows: userRows } = await db.query('SELECT password FROM users WHERE id = $1 LIMIT 1', [userId]);
        ...
        if (passwordChanged && !fullname && !phone) message = "Password changed successfully";
        else if (passwordChanged) message = "Profile and password updated successfully";
...
export const logout = async (req, res) => {
    try {
        ...
        if (!accessToken)
            throw ApiError.badRequest("Access token not found.");
