...
export const getProducts = async (req, res) => {
    try {
        ...
        if (search) {
            querySegments.push(`(p.name ILIKE $${index} OR p.description ILIKE $${index})`);
            queryValues.push(`%${search}%`);
            index++;
        }

        if (is_popular === 'true')
            querySegments.push(`p.is_popular = true`);

        if (is_new === 'true')
            querySegments.push(`p.is_new = true`);
...
export const getProductById = async (req, res) => {
    try {
        ...
        const optionsQuery = `
            SELECT id, option_type, name, extra_price
            FROM product_options
            WHERE product_id = $1 AND is_available = true
            ORDER BY extra_price ASC
        `;
        const { rows: optionRows } = await db.query(optionsQuery, [id]);
