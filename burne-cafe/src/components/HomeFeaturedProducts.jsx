...
function HomeFeaturedProducts() {
    // Kategorilere göre çeşitli popüler ürünler seç
    ...
    // 2 sıcak + 1 soğuk/frappe + 1 serinletici
        ...
        <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            ...
            <Link to={`/urun/${product.id}`} className="block relative">
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-2">
                    ...
                    {product.isNew && (
                        <span className="h-7 px-2 flex items-center justify-center bg-[#C46A2B] text-white text-xs font-medium rounded-md">
                            Yeni
                        </span>
                    )}
                    {hasDiscount && (
                        <span className="h-7 px-2 flex items-center justify-center bg-[#8B4513] text-[#F5DEB3] text-xs font-bold rounded-md border border-[#D4A574]/30">
                            %{product.discount}
                        </span>
                    )}
                </div>
