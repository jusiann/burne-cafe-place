...
*-* Auth: Guest checkout (geçmiş yok) + email/şifre kayıt (geçmiş var)
...
> 2. **Checkout** — LocationContext'teki il/ilçeye göre eşleşen şube gösterilir ("Siparişin buraya gelecek")

- GET /api/branches?city=&district= — aktif şubeler (public)
- GET /api/branches/:id — şube detay

### 3.4 Coupons (validate)
- POST /api/coupons/validate — { code, cartItems, userId? } → sunucu tarafında doğrulama
  - ILK15: userId varsa order count=0 kontrolü, guest için false
  - IKILIM20: cartItems'da productId 11 ve 12 var mı
  - MIEL10: cartItems'da productId 1 var mı
...
- POST /api/orders — hem guest hem auth. Body: branchId, customerName, customerPhone, items[], scheduledTime?, paymentMethod, orderNote?, couponCode?
...
### 6.1 AuthContext (`web/src/context/AuthContext.jsx`)
- State: user, token, isLoading
- Methods: login(), register(), logout()
- Token: localStorage'da tutulur, axios interceptor ile header'a eklenir

### 6.2 LocationContext (`web/src/context/LocationContext.jsx`)
- State: { city, district }
...
- Seçim yapılınca LocationContext günceller

### 6.4 Login.jsx & Register.jsx Pages
- Login: email + şifre → POST /api/auth/login → AuthContext.login()
- Register: isim, email, şifre, telefon → POST /api/auth/register → otomatik login

### 6.5 Navbar Güncellemesi
- LocationContext'ten konum göster: "📍 İstanbul / Kartal"
- AuthContext'e göre: login/register butonları veya kullanıcı adı + logout
- Konum tıklanınca OnboardingModal açılır

### 6.6 ProtectedRoute Component
- role prop alır
- AuthContext.user.role kontrolü
...
### 7.2 CartContext Güncellemesi
- Cart: localStorage'da aynı kalır (client-side)
- `createOrder()` → POST /api/orders çağrısı
...
- Şube gösterimi eklenir (LocationContext'ten otomatik → matching branch gösterilir)
...
└── web/
    ...
    └── src/
        ...
        ├── context/
        │   ├── CartContext.jsx   ← API entegrasyonu
        │   ├── AuthContext.jsx   ← YENİ
        │   └── LocationContext.jsx ← YENİ
        ├── services/
...
- branches 1—* orders
- users *—* branches (staff_branches join table)
