...
import { User, Phone, MapPin, Clock, CreditCard, FileText, ShoppingBag, Tag, AlertCircle } from 'lucide-react';
...
const InputField = ({ name, label, icon: Icon, type = 'text', required = true, formData, errors, handleChange, ...props }: InputFieldProps) => (
    <div>
...
function CheckoutSection() {
    ...
    const timeOptions = useMemo(() => {
        ...
        for (let hour = startHour; hour < 24; hour++) {
            ...
            options.push({ value: `${hour}:00`, label: `${displayHour}:00` });
        }
        return options;
    }, []);

    const paymentMethods: { value: string; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-white rounded-xl border border-[#E8E0D5] p-6">
                            ...
                            <div className="grid sm:grid-cols-2 gap-4">
                                <InputField name="customerName" label="Ad Soyad" icon={User} placeholder="Adınız ve soyadınız" formData={formData} errors={errors} handleChange={handleChange as React.ChangeEventHandler<HTMLInputElement>} />
                                <div>
                                    <label htmlFor="customerPhone" className="block text-sm font-medium text-[#2B1E17] mb-2">Telefon <span className="text-[#C46A2B]">*</span></label>
                                    <div className="relative">
                                    ...
                                    {errors.customerPhone && <p className="mt-1 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.customerPhone}</p>}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl border border-[#E8E0D5] p-6">
                            ...
                            <div className="space-y-4">
                                <div className="grid sm:grid-cols-3 gap-4">
                                    <InputField name="city" label="Şehir" icon={MapPin} placeholder="İstanbul" formData={formData} errors={errors} handleChange={handleChange as React.ChangeEventHandler<HTMLInputElement>} />
                                    <InputField name="district" label="İlçe" icon={MapPin} placeholder="Kadıköy" formData={formData} errors={errors} handleChange={handleChange as React.ChangeEventHandler<HTMLInputElement>} />
                                    <InputField name="neighborhood" label="Mahalle" icon={MapPin} placeholder="Moda" formData={formData} errors={errors} handleChange={handleChange as React.ChangeEventHandler<HTMLInputElement>} />
                                </div>
                                <div>
                                    <label htmlFor="fullAddress" className="block text-sm font-medium text-[#2B1E17] mb-2">Tam Adres <span className="text-[#C46A2B]">*</span></label>
                                    <textarea
                                        id="fullAddress"
                                        name="fullAddress"
                                    ...
                                    />
                                    {errors.fullAddress && <p className="mt-1 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.fullAddress}</p>}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="space-y-4">
                                <div className="grid sm:grid-cols-2 gap-3">
                                    {deliveryTimeOptions.map((option) => (
                                        <label key={option.value} className={`flex items-center p-3 border-2 rounded-xl cursor-pointer transition-all ${formData.deliveryTime === option.value ? 'border-[#C46A2B] bg-[#C46A2B]/5' : 'border-[#E8E0D5] hover:border-[#C46A2B]/50'}`}>
                                            <input
                                                type="radio"
                                                name="deliveryTime"
                                                value={option.value}
                                ...
                                {formData.deliveryTime === 'custom' && (
                                    <div>
                                        <label className="block text-sm font-medium text-[#2B1E17] mb-2">Teslimat Saati Seçin <span className="text-[#C46A2B]">*</span></label>
                                        <select
                                            name="customTime"
                                        ...
                                        {errors.customTime && <p className="mt-1 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.customTime}</p>}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-xl border border-[#E8E0D5] p-6">
                            ...
                            <div className="space-y-4">
                                {paymentMethods.map((method) => (
                                    <label key={method.value} className={`flex items-center p-4 border-2 rounded-xl cursor-pointer transition-all ${formData.paymentMethod === method.value ? 'border-[#C46A2B] bg-[#C46A2B]/5' : 'border-[#E8E0D5] hover:border-[#C46A2B]/50'}`}>
                                        <input
                                            type="radio"
                                            name="paymentMethod"
                                            value={method.value}
                                ...
                                {formData.paymentMethod === 'online_card' && (
                                    <div className="mt-4 p-4 bg-[#F5F1EB] rounded-xl space-y-4">
                                        <div>
                                            <label className="block text-sm font-medium text-[#2B1E17] mb-2">Kart Numarası <span className="text-[#C46A2B]">*</span></label>
                                            <input
                                                type="text"
                                                name="cardNumber"
                                            ...
                                            />
                                            {errors.cardNumber && <p className="mt-1 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.cardNumber}</p>}
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-[#2B1E17] mb-2">Kart Üzerindeki İsim <span className="text-[#C46A2B]">*</span></label>
                                            <input
                                                type="text"
                                                name="cardName"
                                            ...
                                            />
                                            {errors.cardName && <p className="mt-1 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.cardName}</p>}
                                        </div>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="block text-sm font-medium text-[#2B1E17] mb-2">Son Kullanma <span className="text-[#C46A2B]">*</span></label>
                                                <input
                                                    type="text"
                                                    name="cardExpiry"
                                                ...
                                                />
                                                {errors.cardExpiry && <p className="mt-1 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.cardExpiry}</p>}
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-[#2B1E17] mb-2">CVV <span className="text-[#C46A2B]">*</span></label>
                                                <input
                                                    type="text"
                                                    name="cardCvv"
                                                ...
                                                />
                                                {errors.cardCvv && <p className="mt-1 text-sm text-red-600 flex items-center gap-1"><AlertCircle className="w-4 h-4" />{errors.cardCvv}</p>}
                                            </div>
                                        </div>
                                    </div>
                                )}
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-xl border border-[#E8E0D5] p-6 sticky top-24">
                            ...
                            <div className="space-y-2 mb-6">
                                <div className="flex justify-between text-sm text-[#2B1E17]"><span>Ara Toplam</span><span>₺{cartTotals.subtotal.toFixed(2)}</span></div>
                                {appliedCoupon && cartTotals.discount > 0 && (
                                    <div className="flex justify-between text-sm text-[#C46A2B]">
                                        <div className="flex items-center gap-1"><Tag className="w-4 h-4" /><span>{appliedCoupon.code}</span></div>
                                        <span>-₺{cartTotals.discount.toFixed(2)}</span>
                                    </div>
                                )}
                                <div className="flex justify-between text-sm text-[#8B7E75]"><span>KDV (%20)</span><span>₺{cartTotals.tax.toFixed(2)}</span></div>
