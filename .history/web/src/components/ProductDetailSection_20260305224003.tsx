...
import { Minus, Plus, Check, Coffee, Droplet, Star, Flame, Dumbbell, Bean, ChevronLeft } from 'lucide-react';
import { useCart } from '../context/CartContext';
import type { Product, Extra } from '../types/product';
...
const NutritionItem = ({ icon: Icon, label, value, unit }: NutritionItemProps) => (
...
function ProductDetailSection({ product }: ProductDetailSectionProps) {
    const { addToCart, updateItem } = useCart();
    const navigate = useNavigate();
    const location = useLocation();

    const [selectedSize, setSelectedSize] = useState<import('../types/product').ProductSize | null>(product.sizes?.[0] ?? null);
    const [selectedMilk, setSelectedMilk] = useState<import('../types/product').MilkOption | null>(product.milkOptions?.[0] ?? null);
    ...
    /* INITIALIZATION */
    useEffect(() => {
        const editItem = (location.state as { cartItem?: CartItem } | null)?.cartItem;
        if (editItem && String(editItem.productId) === String(product.id)) {
            ...
            if (editItem.extras && product.extras) {
                const restoredExtras = editItem.extras
                    .map(e => product.extras?.find(pe => pe.name === e.name))
            ...
            setEditingItem(null);
            const defaultSize = product.sizes?.find(s => s.name === 'Tall') ?? product.sizes?.[0] ?? null;
            ...
            if (product.milkOptions && product.milkOptions.length > 0) {
                ...
                setSelectedMilk(defaultMilk ?? null);
            }
            setSelectedExtras([]);
            setQuantity(1);
            setNote('');
    ...
    const handleToggleExtra = (extra: Extra) => {
        setSelectedExtras(previous => {
            const isSelected = previous.find(e => e.name === extra.name);
            return isSelected ? previous.filter(e => e.name !== extra.name) : [...previous, extra];
        });
    };
    ...
    return (
        <section className="pt-6 pb-8 md:py-16 overflow-x-hidden">
            <div className="w-full max-w-6xl mx-auto px-5 sm:px-6 lg:px-8">
                ...
                {/* BACK LINK */}
                <Link to="/menu" className="inline-flex items-center gap-2 text-[#8B7E75] hover:text-[#C46A2B] mb-8 transition-colors text-lg font-heading uppercase tracking-wide">
                    ...
                    <div className="space-y-6">
                        <div className="aspect-square w-full rounded-2xl overflow-hidden bg-[#F5F1EB] border border-[#E8E0D5] relative md:sticky md:top-24">
                            ...
                            <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
                                ...
                                {product.discount > 0 && (
                                    <span className="h-8 px-3 flex items-center justify-center bg-[#8B4513] text-[#F5DEB3] text-sm font-bold rounded-lg border border-[#D4A574]/50 shadow-lg">%{product.discount}</span>
                                )}
                            </div>
                        </div>
                    </div>
                                        ...
                                        {product.extras.map((extra) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center justify-between p-3 rounded-xl border transition-all ${isSelected ? 'border-[#C46A2B] bg-[#C46A2B]/5 text-[#C46A2B]' : 'border-[#E8E0D5] text-[#8B7E75] hover:bg-[#F5F1EB]'}`}
                                                >
                        ...
                        <div className="mt-4 w-full p-0 overflow-hidden">
                            <div className="flex gap-2 sm:gap-4">
                                <div className="flex items-center bg-[#C46A2B] rounded-xl p-1 shrink-0">
                                    <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-10 h-10 flex items-center justify-center hover:bg-[#A85A24] rounded-lg text-white transition-all disabled:opacity-50" disabled={quantity <= 1}>
                                        <Minus className="w-4 h-4" />
                                    </button>
                                    <span className="w-10 text-center font-bold text-lg text-white">{quantity}</span>
                                    <button onClick={() => setQuantity(quantity + 1)} className="w-10 h-10 flex items-center justify-center hover:bg-[#A85A24] rounded-lg text-white transition-all">
                                ...
                                <button
                                    ...
                                    disabled={isAdded}
                                    className={`flex-1 min-w-0 py-3 px-4 sm:px-6 rounded-xl font-semibold text-sm sm:text-base flex items-center justify-center gap-2 transition-all transform active:scale-95 ${isAdded ? 'bg-[#A85A24] text-white' : 'bg-[#C46A2B] text-white hover:bg-[#A85A24]'}`}
                                >
                                    {isAdded ? (
                                        ...
                                        <>
                                            ...
                                            <span className="bg-white/20 px-2 py-0.5 rounded text-sm ml-1">₺{(totalPrice * quantity).toFixed(2)}</span>
                                        </>
                                    )}
                                </button>
                            </div>
