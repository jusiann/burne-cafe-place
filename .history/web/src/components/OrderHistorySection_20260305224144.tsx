...
const statusIcons: Record<OrderStatus, StatusConfig> = {
...
function OrderHistorySection() {
    ...
    const openConfirmation = (orderId: string) => {
        ...
        <section className="py-8">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="space-y-4">
                    {orders.map((order) => {
                        const statusConfig = statusIcons[order.status as OrderStatus];
                        ...
                        return (
                            <div key={order.id} className="bg-white rounded-xl border border-[#E8E0D5] overflow-hidden hover:shadow-lg transition-shadow">
                                ...
                                <span className="hidden"><StatusIcon /></span>
                            </div>
                        );
                    })}
                </div>
                ...
                {confirmationModal.isOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                            ...
                            <div className="p-6">
                                <p className="text-[#8B7E75] text-base leading-relaxed">Bu siparişi iptal etmek istediğinizden emin misiniz? Bu işlem geri alınamaz.</p>
                            </div>
                            <div className="px-6 py-4 bg-[#F5F1EB]/50 border-t border-[#E8E0D5] flex justify-end gap-3">
                                <button onClick={closeConfirmation} className="px-4 py-2 rounded-xl text-[#8B7E75] font-medium hover:bg-[#E8E0D5]/50 transition-colors">Vazgeç</button>
                                <button
                                    onClick={() => {
                                        if (confirmationModal.orderId) {
