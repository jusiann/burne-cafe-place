      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-1">
            {navItems.map((item) => (
              <Link
                ...
                to={item.href}
                className={cn(
                  'relative px-4 py-2 font-medium transition-all duration-300 rounded-lg border-b-2',
                  isActiveRoute(item.href)
                    ? 'text-[#C46A2B] bg-[#C46A2B]/10 border-[#C46A2B]'
                    : 'text-[#2B1E17] hover:text-[#C46A2B] hover:bg-[#C46A2B]/5 border-transparent'
                )}
              >
                {item.name}
              </Link>
            ...
            <button
              onClick={toggleCart}
              className="relative p-2 ml-2 rounded-lg transition-all duration-300 group text-[#2B1E17] hover:text-[#C46A2B] hover:bg-[#C46A2B]/5"
          ...
          <div className="flex items-center gap-2 md:hidden">
            ...
            <button
              onClick={toggleCart}
              className="relative p-2 rounded-lg transition-all duration-300 text-[#2B1E17] hover:text-[#C46A2B]"
              aria-label={`Sepet - ${totalItems} ürün`}
            >
        ...
        <div className="px-4 py-4 space-y-2">
          {navItems.map((item) => (
            <Link
              ...
              to={item.href}
              className={cn(
                'block px-4 py-3 font-medium rounded-lg transition-all duration-300 border-l-2',
                isActiveRoute(item.href)
                  ? 'text-[#C46A2B] bg-[#C46A2B]/5 border-[#C46A2B]'
                  : 'text-[#2B1E17] hover:text-[#C46A2B] hover:bg-[#C46A2B]/5 border-transparent'
              )}
              onClick={() => setIsMenuOpen(false)}
            >
          ...
          <button
            ...
            className="flex items-center justify-between w-full px-4 py-3 font-medium rounded-lg transition-all duration-300 border-l-2 text-[#2B1E17] hover:text-[#C46A2B] hover:bg-[#C46A2B]/5 border-transparent"
          >
