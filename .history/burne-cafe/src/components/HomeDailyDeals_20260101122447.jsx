    ...
    return (
        <section className="py-20 bg-gradient-to-br from-[#2B1E17] via-[#3D2B20] to-[#2B1E17] relative overflow-hidden">
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="mt-12 p-6 md:p-8 bg-gradient-to-r from-[#C46A2B] to-[#A85A24] rounded-2xl max-w-4xl mx-auto relative overflow-hidden">
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-start gap-4 mb-4">
                                                <Percent className="w-10 h-10 md:w-12 md:h-12 text-white flex-shrink-0" />
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-white/90 text-sm font-medium mb-1">{details?.title}</p>
                                                    <p className="text-white font-bold text-xl md:text-2xl mb-2">{details?.description}</p>
                                                    <p className="text-white/80 text-xs md:text-sm leading-relaxed">{details?.conditions}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                                                <button
                                                    ...
                                                    className="relative flex-1 sm:flex-initial bg-white/20 backdrop-blur-sm px-4 py-3 rounded-xl border border-white/30 hover:bg-white/30 transition-all duration-300 cursor-pointer group"
                                                >
                                                    <p className="text-white text-sm text-center">
                                                        <span className="font-normal">Kupon Kodu: </span>
                                                        <span className="font-bold text-base tracking-wider">{coupon.code}</span>
                                                    </p>
                                                    {copiedCode === coupon.code && (
                                                        <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-white text-[#C46A2B] text-xs px-2 py-1 rounded shadow-lg whitespace-nowrap">
                                                            Kopyalandı!
                                                        </span>
                                                    )}
                                                </button>
                                                <Link
                                                    to="/menu"
                                                    className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 px-6 py-3 bg-white text-[#C46A2B] font-semibold rounded-xl hover:bg-white/90 hover:shadow-2xl transition-all duration-300"
                                                >
                                                    Sipariş Ver
                            ...
                            {coupons.map((_, index) => (
                                <button
                                    ...
                                    className={`
                                        ...
                                        duration-500
                                        cursor-pointer
                                        ${index === activeCouponIndex ? 'h-14 md:h-16' : 'h-7 md:h-8'}
                                    `}
                                >
                                    <span
                                        className={`
